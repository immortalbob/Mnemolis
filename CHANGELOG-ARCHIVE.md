# Changelog Archive (v1.0.0 – v3.44.1)

The full, unedited changelog for every version before the documentation/investigation methodology shift that begins at v3.45.0. Kept verbatim and complete — this file is the historical record, not a summary of it.

Two ranges, by character, within this archive itself:

- **v1.0.0 – v3.18.2**: pure feature construction. New sources, new endpoints, caching, MCP, API key auth — terse, mostly bullet-point entries with little narrative, because there wasn't yet a deliberate bug-hunting discipline generating one.
- **v3.19.0 – v3.44.1**: a real, distinct methodology begins — static-analysis-tool-driven complexity refactoring (v3.19–v3.22, see [The Complexity Refactor Campaign](https://github.com/immortalbob/Mnemolis/wiki/The-Complexity-Refactor-Campaign)), the MCP transport migration (v3.19, see [The MCP Transport Migration](https://github.com/immortalbob/Mnemolis/wiki/The-MCP-Transport-Migration)), a battle-testing campaign, and a full bulletproofing pass across every source file (v3.35–v3.44, see [The Home Assistant](https://github.com/immortalbob/Mnemolis/wiki/The-Home-Assistant-Bulletproofing-Pass), [Kiwix](https://github.com/immortalbob/Mnemolis/wiki/The-Kiwix-Bulletproofing-Pass), and [Snapshot Engine](https://github.com/immortalbob/Mnemolis/wiki/The-Snapshot-Engine-Bulletproofing-Pass) bulletproofing pages). v3.44.1 itself is the project's own checkpoint marking both campaigns complete, right before the next era (Adversarial Self-Testing, Cross-Source Temporal Pattern Detection, the latency-parallelization investigation, and everything in the live `CHANGELOG.md`) begins at v3.45.0.

Every substantial, multi-bug investigation from this range that didn't already have a dedicated wiki page has one now — written directly from this archive's own entries, not summarized from memory. See [Home](https://github.com/immortalbob/Mnemolis/wiki/Home#design-history--real-bugs-real-fixes) for the full Design History index, which includes pages sourced from both this archive and the live changelog.

For everything from v3.45.0 onward, see `CHANGELOG.md` in the repo root.
