...
The underlying `uptime_kuma_api` library also pays a real, brief grace period (`wait_events`, 0.2s by default) after each call, to let any trailing per-monitor data pushes land before treating a batch as complete — necessary for the very first call after a fresh connect/login, but a real, avoidable cost on every later call if left unchanged. Mnemolis keeps the library's full, safe default for that one critical first call and shortens it for everything afterward on the same connection — see this page's Development Notes below for the full investigation.
...
- **`uptime` used to open a brand-new Socket.IO connection, log in, and disconnect on every single call** — including every 2-minute `snapshot_uptime()` scheduler tick, independent of whether anyone was ever asking a live `uptime` question. This was a real, confirmed contributor to a warm-cache latency tail that had reproduced across three separate benchmark releases (v3.17.0, v3.44.0, v3.50.2). Fixed (v3.50.4) by making the connection persistent, reused across calls, with the underlying library's own confirmed liveness check (`sio.connected`, not a `.connected` property `UptimeKumaApi` doesn't actually have) used to detect and transparently replace a dead connection rather than reuse it silently broken. This measurably helped (warm p95 1500ms → 470ms) but left a smaller, real tail that reproduced identically across two further benchmark runs (v3.50.6, v3.50.7), with no confirmed root cause until directly reproducing the library's behavior in isolation (v3.50.8) found it: `uptime_kuma_api`'s own `_get_event_data()` pays an unconditional 0.2-second `wait_events` sleep on every call, even when the awaited data was already complete — a real, fixed library cost the persistent-connection fix never addressed, since it has nothing to do with the connection itself staying open. Fixed by shrinking that wait once a connection's first call has genuinely settled, while keeping the library's full, safe default for that one first call, where it's still genuinely needed (Uptime Kuma sends one `heartbeatList` push per monitor right after login, and the wait exists to let all of them land before the batch is considered complete). **Confirmed by a real re-benchmark (v3.50.9): warm p98/p99 dropped to 69ms**, finally in the same range as the cleanest sources rather than a separate tail — this one is fully resolved, not just verified in tests. See [The Benchmark Investigation Log](The-Benchmark-Investigation-Log#thread-1-uptimes-warm-cache-tail-five-releases-to-a-real-root-cause) for the full, honest history across all five releases, and [Caching](Caching#why-uptimes-connection-is-persistent-even-though-its-cache-ttl-stays-short) for why the first fix was a connection-level change rather than a `CACHE_TTL_UPTIME_SECONDS` one.
