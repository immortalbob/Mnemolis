...
The real fix was at the connection level, not the cache level: the Socket.IO connection to Uptime Kuma is now persistent, established once and reused across every call — see [Sources](Sources#uptime--service-monitoring) for the mechanism. `CACHE_TTL_UPTIME_SECONDS` itself is completely unchanged by this; a cache miss still happens exactly as often as the TTL dictates. The real-world benchmark confirms this made the large majority of calls genuinely cheap, in line with every other warm source — but not all of them; see [Benchmarks](Benchmarks#a-partially-confirmed-fix-a-partially-effective-mitigation-and-one-fix-confirmed-holding-under-load) for the honest, measured result and what's still unexplained.
...
- **`uptime`'s warm-cache tail looked like a caching problem for three releases before it was confirmed to actually be a connection-lifecycle problem — and the fix for that turned out to be real, but only a partial explanation.** `CACHE_TTL_UPTIME_SECONDS` being the only source TTL shorter than a typical benchmark run window correctly explained why a cache miss happened during every run — it never explained why that miss cost 1.5-1.9 real seconds. Reading `app/sources/uptime_kuma.py` directly (rather than continuing to reason from benchmark timing alone) found a real cause: a fresh Socket.IO connect+login cycle on every single call, cache miss or not. Making the connection persistent instead of touching the TTL (see the section above) measurably helped — warm p95 dropped from 1500ms to 470ms — but didn't fully close the gap; a smaller, real tail remains with no confirmed root cause yet.
