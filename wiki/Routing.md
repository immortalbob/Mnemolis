...
Rather than trying to nudge the LLM's judgment by rewording `kiwix`'s description — an indirect, hard-to-verify lever — this is detected explicitly with a small set of literal phrase matches (`"everyone keeps talking about"`, `"everyone's obsessed with"`, and a few variants). If one of these phrases is present and `kiwix` wasn't already part of the chosen source(s), `kiwix` gets added and the result escalates to fusion. The pattern list itself is defined once, in `kiwix.py` — not duplicated here — since `kiwix.py` independently needs the same list to strip discourse-framing noise out of its own search terms; see [The Discourse-Framing Investigation](The-Discourse-Framing-Investigation#a-real-deliberate-single-source-of-truth) for why a shared list living in one specific file, rather than two independently-maintained copies, was a deliberate choice with real history behind it.
