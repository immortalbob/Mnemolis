...
- **The MCP `search` tool never sets `isError=True`, even on a genuine internal failure.** Traced directly against the installed SDK: an uncaught exception inside a tool function *would* get converted to a real, protocol-level `CallToolResult(isError=True, ...)` one layer up — the mechanism exists and works. `search`'s own `try/except` catches everything first and returns a normal-looking response with `"Error: ..."` text instead, the same convention every source file in this project already uses for expected, recoverable failures. The real, currently-undecided question: should a *genuinely unexpected* internal error (a real bug, not a source returning its own "not configured" string) be allowed to surface as `isError=True` instead, giving a real MCP client a structured way to tell "Mnemolis answered with bad news" apart from "the call itself broke"? Not changed — this is a real contract change for any existing integration, not a bug fix, and deserves a deliberate decision rather than a one-off patch made while documenting the current behavior. See [MCP Server](MCP-Server#a-real-difference-worth-knowing-about) for the full, precise trace of what the SDK actually does today.
