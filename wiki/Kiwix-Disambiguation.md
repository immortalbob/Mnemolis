...
- **A bad LLM response used to get permanently stuck for the rest of the cache's TTL** — the third occurrence of the same routing-cache bug pattern found twice before in `router.py`. See [The Cached-Failure Bug, Found Three Times](The-Cached-Failure-Bug-Found-Three-Times) for the full account, including the real distinction this occurrence needed that the first two didn't.
