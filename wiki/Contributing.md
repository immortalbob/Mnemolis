...
**Be honest about what a fix doesn't solve.** Several pages in this wiki explicitly document a fix's accepted limitations alongside its success — [Kiwix Disambiguation](Kiwix-Disambiguation#a-genuine-accepted-limitation) and [The Benchmark Investigation Log](The-Benchmark-Investigation-Log)'s recurring anomalies (some fully fixed and re-confirmed by a later benchmark; some genuinely partial, with a real, honestly-stated remaining gap; one sizing pass that shipped a real mistake, caught by the very next re-benchmark rather than hidden; none retrofitted into a cleaner-sounding conclusion than the actual numbers support) are all left in the record on purpose, rather than smoothed over. A PR description that says "this handles X and Y, but I haven't verified Z" is more useful and more trustworthy than one that implies complete coverage.
