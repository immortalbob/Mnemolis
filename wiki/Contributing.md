...
**Be honest about what a fix doesn't solve.** Several pages in this wiki explicitly document a fix's accepted limitations alongside its success — [Kiwix Disambiguation](Kiwix-Disambiguation#a-genuine-accepted-limitation) and the [Benchmarks](Benchmarks#a-now-explained-finding-one-with-a-real-hypothesis-for-the-first-time-and-one-fresh-finding) page's recurring anomalies (one traced to a real, identifiable cause across two releases; one recurring a third time with a real, testable hypothesis proposed for the first time, not yet confirmed; one freshly found and not yet root-caused at all) are all left in the record on purpose, rather than smoothed over. A PR description that says "this handles X and Y, but I haven't verified Z" is more useful and more trustworthy than one that implies complete coverage.
