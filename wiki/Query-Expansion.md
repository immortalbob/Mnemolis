...
When expansion fires, `search()` needs the primary SearXNG fetch, a full LLM completion call (`get_alternate_phrasing()`), and a *second* SearXNG fetch for the alternate phrasing. These run concurrently via a shared, module-level thread pool rather than one after another — but they didn't always. A real latency cost from running them sequentially (roughly 4x the cost of a single fetch) was found via a live Adversarial Self-Testing flag, and fixing it surfaced a genuine, separate file-write race in how the routing cache persists to disk. See [The Latency Parallelization Investigation](The-Latency-Parallelization-Investigation#the-web-case-was-fixed-first) for the full story.

The thread pool itself used to be created fresh on every single call — the identical unbounded-per-call pattern [The Fusion Merge Bugs](The-Fusion-Merge-Bugs#unbounded-thread-creation-and-a-recurring-remotedisconnected-mystery) already found and fixed in `fusion.py`, just never applied here until a later, deliberate function-by-function read found it. Fixing it also fixed a second, related bug: a primary-fetch timeout's early return used to silently wait for the unrelated alternate-phrasing thread to finish before actually reaching the caller, since `return` from inside the old `with ThreadPoolExecutor(...)` block didn't propagate until the block's own implicit shutdown completed. Both are fixed by the same shared-pool change — see `CHANGELOG.md`'s v3.50.27 entry for the full investigation and the measured before/after.
