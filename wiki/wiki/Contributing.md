...
**Be honest about what a fix doesn't solve.** Several pages in this wiki explicitly document a fix's accepted limitations alongside its success — [Kiwix Disambiguation](Kiwix-Disambiguation#a-genuine-accepted-limitation) and the [Benchmarks](Benchmarks#a-now-explained-finding-and-one-still-open-one) page's unexplained anomalies (one since traced to a real, identifiable cause; one still genuinely open after recurring across two separate releases) are both left in the record on purpose, rather than smoothed over. A PR description that says "this handles X and Y, but I haven't verified Z" is more useful and more trustworthy than one that implies complete coverage.
