...
Five real bugs were found in this engine during a deliberate review — a per-source retention gap that left `uptime` with under 10 hours of real history instead of a full week, an unrounded fractional-hour value that could reach a user-facing message, a pending/retry transition reported with the same wording as a confirmed outage, sub-zero temperatures silently breaking change detection entirely, and one malformed entity that could crash the diff for every other entity in the same snapshot. See [The Snapshot Engine Bulletproofing Pass](The-Snapshot-Engine-Bulletproofing-Pass) for all five.
