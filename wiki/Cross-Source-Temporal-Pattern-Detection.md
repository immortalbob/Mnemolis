...
Motion, window, and door/opening sensor states are all tracked here alongside locks and battery — the door→motion example at the top of this page, the literal reason this feature exists, is fully testable today. See [The Temporal Pattern Detection Development Bugs](The-Temporal-Pattern-Detection-Development-Bugs#the-features-own-headline-example-was-never-actually-testable-in-the-version-that-first-shipped) for why that wasn't true in the version that first shipped.
...
Three real bugs were found while building this feature, two caught by deliberately testing harder scenarios than the obvious case, one caught by a later review pass rather than a failing test: a non-overlapping occurrence counter that undercounted genuinely distinct pairs, an uptime-event classifier that misclassified both of its own non-outage messages, and a gap that meant this feature's own headline example — door event preceding motion event — was never actually testable in the version that first shipped. See [The Temporal Pattern Detection Development Bugs](The-Temporal-Pattern-Detection-Development-Bugs) for all three.
